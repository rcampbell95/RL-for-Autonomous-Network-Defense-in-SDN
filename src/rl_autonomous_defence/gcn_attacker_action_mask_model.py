...
class GCNAttackerActionMaskModel(TFModelV2):
    ...
    def __init__(
        ...
        super(GCNAttackerActionMaskModel, self).__init__(obs_space, action_space, num_outputs, model_config, name)
        batch_size = 32
        self.num_states = 4
        self.num_actions = 3
        ...
        self.action_space_dims = len(self.action_space) if isinstance(self.action_space, gym.spaces.Tuple) else 1
        ...
        if isinstance(action_space, gym.spaces.Tuple):
            assert self.num_outputs == self.num_nodes + self.num_actions
        ...
        gc_model_inputs = [
            [tf.keras.layers.Input(gc_input.shape[1:], name=f"gc_input_{j}_frame_{i}")
            for j, gc_input in enumerate(x_inp_policy)] for i in range(self.num_frames)
        ]

        actions = tf.keras.layers.Input(shape=(self.num_frames, self.num_outputs), name=f"Actions input")
    ...
    def call_gc_model(self, input_dict, state):
        ...
        #   if len(actions_labels.shape) == 2:
        #       actions_labels = tf.zeros((actions_labels.shape[0], self.num_frames, self.action_space_dims))
        ...
        if isinstance(self.action_space, gym.spaces.Discrete):
            ...
            #raise Exception(f"Unexpected action tensor shape: {actions.shape}")
        elif isinstance(self.action_space, gym.spaces.MultiDiscrete):
            #raise Exception(f"Actions labels shape {actions_labels.shape} and values {actions_labels}")
        ...
        if self.model_config["custom_model_config"]["masked_actions"]:
            ...
            action_mask = mask_attacker_actions(input_dict["obs"], current_obs_features)
